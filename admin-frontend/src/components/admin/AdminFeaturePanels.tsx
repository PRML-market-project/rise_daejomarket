...
import { searchTagIcons } from "@/data/search-tag-icons";
...
function Button({ children, kind = "primary", disabled, className = "", onClick }: ButtonProps) {
  const tone = kind === "primary" ? "bg-[#116543] text-white" : kind === "danger" ? "bg-[#b3261e] text-white" : kind === "outline" ? "border border-[#a1a1a1] bg-[#f8fbf8]" : "bg-[#ebebeb] text-[#19211c]";
  return <button type="button" disabled={disabled} onClick={onClick} className={`flex h-[48px] items-center justify-center rounded-full px-[20px] text-[16px] font-medium transition hover:brightness-[.98] disabled:cursor-default disabled:opacity-45 ${tone} ${className}`}>{children}</button>;
}

function Modal({ title, children, onClose, footer }: { title: string; children: React.ReactNode; onClose?: () => void; footer?: React.ReactNode }) {
  return <div className="admin-fade-in fixed inset-0 z-[70] flex items-center justify-center bg-[rgba(0,0,0,.32)] p-6" role="dialog" aria-modal="true">
    <div className="w-[640px] rounded-[20px] bg-white p-[32px] shadow-xl">
      <div className="flex items-center justify-between"><h2 className="text-[24px] font-bold text-[#0a3825]">{title}</h2>{onClose && <button type="button" aria-label="닫기" onClick={onClose}><X size={24} /></button>}</div>
      <div className="mt-[20px] text-[16px] font-medium leading-[24px]">{children}</div>
      {footer && <div className="mt-[24px] flex gap-[12px]">{footer}</div>}
    </div>
  </div>;
}
...
export function PromotionManager({ onNavigate, onModeApply }: { onNavigate: (target: "store" | "tags") => void; onModeApply: (mode: "길찾기" | "홍보") => void }) {
  ...
  return <>
    <div className="flex h-[60px] items-start justify-between">
      <div className="flex gap-[32px] text-[40px] leading-[60px]"><button className="font-medium text-[#c3c3c3]" onClick={() => navigate("store")}>가게 관리</button><strong className="text-[#0a3825]">홍보 관리</strong></div>
    ...
    <div className="mt-[20px] grid h-[calc(100vh-343px)] min-h-[737px] grid-cols-[360px_minmax(500px,852px)_minmax(480px,596px)] gap-[24px]">
      <aside className="flex min-h-0 flex-col gap-4 rounded-[20px] border border-[#ebebeb] bg-white p-5">
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto">{items.length ? items.map((item, index) => <div key={item.id} className={`w-full rounded-[14px] border p-3 text-left ${selected === item.id ? "border-[#116543] bg-[#cfeadb]" : "border-[#ebebeb]"}`}>
          <button type="button" onClick={() => setSelected(item.id)} className="flex w-full gap-3 text-left"><span className="h-20 w-[46px] overflow-hidden rounded-md"><Poster video={item.type === "video"} src={item.url} fit={item.fit} /></span><span><strong className="text-[16px]">{String(index + 1).padStart(2, "0")}　{item.name}</strong><span className="mt-1 block text-[12px]">{item.type === "image" ? `이미지 · ${item.durationSeconds}초` : "영상 · 재생 종료 후 다음"}</span></span></button>
        ...
        </div>) : <div className="flex h-full flex-col items-center justify-center text-center"><strong>등록된 콘텐츠가 없습니다.</strong><span className="mt-2 text-[14px] text-[#a1a1a1]">이미지나 영상을 추가해주세요.</span></div>}</div>
...
type TagIcon = { label: string; Icon: ComponentType<{ size?: number }> };
function assetIcon(src: string, ratio = 1): TagIcon["Icon"] {
  return function SearchIcon({ size = 28 }) {
    return <img src={src} alt="" style={{ width: size * ratio, height: size * ratio, objectFit: "contain" }} />;
  };
}
const baseIcons: TagIcon[] = searchTagIcons.map((icon) => ({ label: icon.label, Icon: assetIcon(`/search-icons/${icon.asset}.svg`, icon.size / 34) }));
type SearchTag = { id: number; name: string; keywords: string; icon: number; visible: boolean };
...
export function SearchTagManager({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => { getKioskExperience().then((config) => { if (!config.searchTags.length) return; const custom: TagIcon[] = []; const urls: Record<number, string> = {}; const loaded = config.searchTags.map((tag) => { let icon = baseIcons.findIndex((option) => option.label === tag.icon); if (tag.iconUrl) { icon = baseIcons.length + custom.length; custom.push({ label: tag.icon, Icon: assetIcon(absoluteAssetUrl(tag.iconUrl)) }); urls[icon] = tag.iconUrl; } return ({ id: tag.id, name: tag.name, keywords: tag.keywords, icon: icon < 0 ? 4 : icon, visible: tag.visible }); }); setCustomIcons(custom); setIconUrls(urls); setTags(loaded); setSelected(loaded[0]?.id ?? null); }).catch((cause: Error) => setError(cause.message)); }, []);
  ...
  const save = async () => { if (!current?.name.trim()) { setError("이름을 입력하세요."); return; } if (!current.keywords.trim()) { setError("연결 검색어를 입력하세요."); return; } try { const payload: ApiSearchTag[] = tags.map((tag) => ({ id: tag.id, name: tag.name, keywords: tag.keywords, icon: icons[tag.icon]?.label ?? "식당", iconUrl: iconUrls[tag.icon], visible: tag.visible })); const saved = await saveSearchTags(payload); setDirty(false); setError(saved.pendingTranslations ? `한국어 저장 완료. 번역 대기 ${saved.pendingTranslations}개 — Argos 번역 서버 실행 상태 확인 후 다시 저장하거나 서버를 재시작하세요.` : ""); } catch (cause) { setError((cause as Error).message); } };
  ...
  const addIcon = async () => { if (!iconName.trim() || !iconFile) return; try { const uploaded = await uploadSearchIcon(iconFile); const newIndex = icons.length; setCustomIcons((all) => [...all, { label: iconName, Icon: assetIcon(absoluteAssetUrl(uploaded.url)) }]); setIconUrls((all) => ({ ...all, [newIndex]: uploaded.url })); update({ icon: newIndex }); setIconDialog(null); setIconName(""); setIconFile(null); } catch (cause) { setError((cause as Error).message); setIconDialog("failed"); } };
  return <>
    ...
    <div className="mt-5 grid h-[calc(100vh-272px)] min-h-[720px] grid-cols-[360px_minmax(600px,852px)_minmax(480px,596px)] gap-6">
      <aside className="flex min-h-0 flex-col gap-4 rounded-[20px] border border-[#ebebeb] bg-white p-5">
        <div className="flex"><h2 className="flex-1 text-[20px] font-bold">검색 태그</h2><span className="text-[14px]">표시 {visibleCount} / 3</span></div>
        <Button kind="secondary" className="w-full" onClick={addTag}>+ 태그 추가</Button>
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto">
          {tags.length ? tags.map((tag, index) => {
            const Icon = icons[tag.icon]?.Icon ?? baseIcons[4].Icon;
            return <div key={tag.id} className={`w-full rounded-[14px] border p-3 ${selected === tag.id ? "border-[#116543] bg-[#cfeadb]" : "border-[#ebebeb]"}`}>
              <button type="button" onClick={() => setSelected(tag.id)} className="flex w-full gap-3 text-left">
                <span className="flex h-10 w-10 items-center justify-center rounded-full text-white" style={{ backgroundColor: searchTagIcons[tag.icon]?.color ?? "#ff9500" }}><Icon size={34} /></span>
                <span><strong>{tag.name}</strong><span className="block text-[12px]">표시 · {tag.keywords || "연결 검색어 없음"}</span></span>
              </button>
              <div className="mt-3 flex gap-2">
                <Button kind="secondary" disabled={index === 0} className="h-8 flex-1 px-2 text-[14px]" onClick={() => move(index, -1)}>위로</Button>
                <Button kind="secondary" disabled={index === tags.length - 1} className="h-8 flex-1 px-2 text-[14px]" onClick={() => move(index, 1)}>아래로</Button>
                <Button kind="secondary" className="h-8 flex-1 px-2 text-[14px]" onClick={() => { setSelected(tag.id); setDeleteOpen(true); }}>삭제</Button>
              </div>
            </div>;
          }) : <div className="flex h-full flex-col items-center justify-center"><strong>등록한 검색 태그가 없어요.</strong><span className="mt-2 text-[#a1a1a1]">위의 ‘태그 추가’를 눌러주세요.</span></div>}
        </div>
        <p className="text-[12px]">위에서부터 순서대로 표시합니다.<br />숨긴 태그는 키오스크에 표시하지 않습니다.</p>
      </aside>
      <section className="rounded-[20px] border border-[#ebebeb] bg-white p-5"><div className="flex"><h2 className="flex-1 text-[20px] font-bold">키오스크 미리보기</h2><span className="text-[14px]">한국어</span></div><div className="mt-5 rounded-[20px] bg-[#f8fbf8] p-5"><div className="flex gap-2"><Button className="w-32">한국어</Button><Button kind="secondary" className="w-32">English</Button><Button kind="secondary" className="w-32">Tiếng Việt</Button></div><div className="mt-5 flex h-[92px] items-center gap-4 rounded-full bg-[#ebebeb] px-8 text-[28px] text-[#a1a1a1]"><Search size={32} />검색어를 입력하세요</div><div className="mt-5 flex gap-3">{previewTags.map((tag) => { const Icon = icons[tag.icon]?.Icon ?? baseIcons[4].Icon; return <span key={tag.id} className="flex items-center gap-2 rounded-full border border-[#d9dfda] bg-white px-4 py-2 text-[22px]"><i className="flex h-8 w-8 items-center justify-center rounded-full text-white" style={{ backgroundColor: searchTagIcons[tag.icon]?.color ?? "#ff9500" }}><Icon size={27.2} /></i>{tag.name}</span>; })}</div></div><div className="mt-5 rounded-xl bg-[#f8fbf8] p-5 text-[14px]"><strong>태그를 누르면 연결된 검색어로 바로 검색합니다.</strong><p className="mt-2">예: 주반식당 → 음식점 검색<br />가게 관리에 등록된 검색용 키워드를 기준으로 찾습니다.</p></div><p className="mt-5 text-[14px] text-[#a1a1a1]">태그의 이름·아이콘·순서를 미리 확인하세요.<br />저장 전 변경은 실제 키오스크에 반영되지 않습니다.</p></section>
      <section className="flex min-h-0 flex-col rounded-[20px] border border-[#ebebeb] bg-white p-5"><div className="flex"><h2 className="flex-1 text-[32px] font-bold">태그 설정</h2><span className="rounded-full bg-[#cfeadb] px-3 py-1 text-[14px]">{current?.name === "새 태그" ? "새 태그" : dirty ? "수정 중" : "저장됨"}</span></div>{current ? <div className="mt-8 min-h-0 flex-1 space-y-4 overflow-y-auto"><label className="block text-[14px] font-bold">이름<input value={current.name} onChange={(e) => update({ name: e.target.value })} className={`mt-2 h-12 w-full rounded-xl border p-3 text-[16px] outline-none ${error.includes("이름") ? "border-[#b3261e]" : "border-[#ebebeb]"}`} /></label><label className="block rounded-xl bg-[#f6f6f6] p-4 text-[14px] font-bold">연결 검색어 *<input value={current.keywords} onChange={(e) => update({ keywords: e.target.value })} className="mt-2 h-12 w-full rounded-xl border border-[#ebebeb] bg-white p-3 text-[16px] outline-none" /><span className="mt-2 block font-medium text-[#a1a1a1]">가게 관리에 등록된 검색용 키워드를 입력하세요.</span></label><div className="rounded-xl bg-[#f6f6f6] p-4"><strong className="text-[14px]">아이콘</strong><div className="mt-3 grid grid-cols-4 gap-3">{icons.map(({ label, Icon }, index) => <button type="button" key={`${label}-${index}`} onClick={() => update({ icon: index })} className={`flex flex-col items-center gap-2 rounded-xl border p-2 text-[12px] ${current.icon === index ? "border-2 border-[#116543] bg-[#cfeadb]" : "border-[#e1e5e1] bg-[#f8fbf8]"}`}><span className="flex h-[34px] w-[34px] items-center justify-center rounded-md bg-[#7a7a7a] text-white"><Icon size={34} /></span>{label}</button>)}<button type="button" onClick={() => setIconDialog("empty")} className="flex flex-col items-center justify-center gap-2 rounded-xl border border-[#e1e5e1] bg-[#f8fbf8] p-2 text-[12px] text-[#116543]"><Plus />아이콘 추가</button></div></div><div className="rounded-xl bg-[#f6f6f6] p-4"><strong className="text-[14px]">키오스크 표시</strong><div className="mt-3 flex gap-2"><Button className="flex-1" kind={current.visible ? "primary" : "secondary"} disabled={!current.visible && visibleCount >= 3} onClick={() => update({ visible: true })}>표시</Button><Button className="flex-1" kind={!current.visible ? "primary" : "secondary"} onClick={() => update({ visible: false })}>숨김</Button></div><p className="mt-2 text-[14px] text-[#a1a1a1]">표시할 태그는 최대 3개입니다. 숨긴 태그는 화면에 나오지 않습니다.</p></div></div> : <p className="mt-8 flex-1 font-bold">태그를 추가하거나 목록에서 선택하세요.</p>}<div><p className={`mb-2 text-[14px] ${error ? "text-[#b3261e]" : "text-[#116543]"}`}>{error || (dirty ? "저장하지 않은 변경이 있습니다." : "목록과 설정을 저장하면 키오스크에 반영됩니다.")}</p><div className="flex gap-3"><Button kind="secondary" disabled={!dirty} className="h-14 w-36" onClick={() => { setTags(seedTags); setSelected(1); setDirty(false); setError(""); }}>변경 취소</Button><Button disabled={!current || !dirty} className="h-14 flex-1" onClick={save}>검색 태그 저장</Button></div></div></section>
    </div>
    {deleteOpen && current && <Modal title="이 검색 태그를 삭제할까요?" onClose={() => setDeleteOpen(false)} footer={<><Button kind="secondary" className="h-14 flex-1" onClick={() => setDeleteOpen(false)}>취소</Button><Button kind="danger" className="h-14 flex-1" onClick={() => { setTags((all) => all.filter((x) => x.id !== current.id)); setSelected(tags.find((x) => x.id !== current.id)?.id ?? null); setDirty(true); setDeleteOpen(false); }}>목록에서 삭제</Button></>}><p>{current.name}<br />목록에서 삭제한 뒤 ‘검색 태그 저장’을 눌러야 반영됩니다.</p></Modal>}
    {iconDialog && <Modal title="아이콘 추가" onClose={() => setIconDialog(null)} footer={<><Button kind="secondary" className="h-14 w-[170px]" onClick={() => setIconDialog(null)}>취소</Button><Button className="h-14 flex-1" disabled={!iconName.trim() || !iconFile || iconDialog === "empty"} onClick={iconDialog === "failed" ? () => fileRef.current?.click() : addIcon}>{iconDialog === "failed" ? "다시 시도" : "추가하고 선택"}</Button></>}><p>검색 태그에 사용할 아이콘을 등록하세요.</p><label className="mt-5 block text-[14px] font-bold">아이콘 이름 *<input value={iconName} onChange={(e) => setIconName(e.target.value.slice(0, 20))} placeholder="이름을 입력하세요 · 최대 20자" className="mt-2 h-12 w-full rounded-xl border border-[#ebebeb] p-3 font-medium" /></label><div className="mt-4 rounded-xl bg-[#f6f6f6] p-5"><strong className="text-[14px]">SVG 또는 투명 PNG · 최대 1MB</strong><p className="mt-1 text-[14px]">투명 배경의 흰색 아이콘을 사용하세요.</p><Button kind="secondary" className="mt-4 w-full" onClick={() => fileRef.current?.click()}>{iconFile ? "다른 파일 선택" : "파일 선택"}</Button><input ref={fileRef} type="file" accept="image/svg+xml,image/png" className="hidden" onChange={chooseIconFile} />{iconFile && <p className={`mt-3 text-[14px] ${iconDialog === "failed" ? "text-[#b3261e]" : "text-[#116543]"}`}>{iconDialog === "failed" ? "업로드하지 못했어요. 다시 시도하거나 다른 파일을 선택해주세요." : `${iconFile.name} · 파일을 확인했어요.`}</p>}</div></Modal>}
  </>;
}
