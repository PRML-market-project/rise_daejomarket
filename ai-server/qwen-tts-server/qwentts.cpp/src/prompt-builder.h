...
static const char * prompt_codec_embedding_name(const PipelineTTS * pt) {
    const char * arch = gf_get_str(pt->gguf_talker, "general.architecture");
    return arch && std::strcmp(arch, "qwen3tts") == 0 ? "talker.token_embd.weight" : "talker.codec_embd.weight";
}
...
static bool prompt_cache_load(PipelineTTS * pt) {
    ...
    embed_row_to_f32(pt->gguf_talker, prompt_codec_embedding_name(pt), pt->codec_specials.pad_id, hidden,
                     pc.codec_pad_emb.data());
    embed_row_to_f32(pt->gguf_talker, prompt_codec_embedding_name(pt), pt->codec_specials.bos_id, hidden,
    ...
    if (prefix_hit) {
        ...
        for (int i = 0; i < (int) codec_left.size(); i++) {
            ...
            if (codec_left[(size_t) i] == -2) {
                ...
                embed_row_to_f32(pt->gguf_talker, prompt_codec_embedding_name(pt), codec_left[(size_t) i], hidden,
                                 ce.data());
            }
