...
static bool code_predictor_weights_load(CodePredictorWeights * cw, const GGUFModel & gf, ggml_backend_t backend) {
    ...
    cw->hidden_size = (int) gf_get_u32(gf, compat ? "qwen3tts.code_pred.d_model"
                                                  : "qwen3-tts.code_pred.embedding_length");
    cw->intermediate_size =
        compat ? 3072 : (int) gf_get_u32(gf, "qwen3-tts.code_pred.feed_forward_length");
    cw->num_hidden_layers = (int) gf_get_u32(gf, compat ? "qwen3tts.code_pred.n_layers"
                                                       : "qwen3-tts.code_pred.block_count");
    cw->num_attention_heads = (int) gf_get_u32(gf, compat ? "qwen3tts.code_pred.n_heads"
                                                          : "qwen3-tts.code_pred.attention.head_count");
    cw->num_key_value_heads = (int) gf_get_u32(gf, compat ? "qwen3tts.code_pred.n_kv_heads"
                                                          : "qwen3-tts.code_pred.attention.head_count_kv");
    cw->head_dim = compat ? 128 : (int) gf_get_u32(gf, "qwen3-tts.code_pred.attention.key_length");
    cw->vocab_size = (int) gf_get_u32(gf, compat ? "qwen3tts.code_pred.vocab_size"
                                                : "qwen3-tts.code_pred.vocab_size");
    cw->rope_theta   = compat ? 1000000.0f : gf_get_f32(gf, "qwen3-tts.code_pred.rope.freq_base");
    cw->rms_norm_eps = compat ? 1e-6f : gf_get_f32(gf, "qwen3-tts.code_pred.attention.layer_norm_rms_epsilon");

    int num_code_groups = (int) gf_get_u32(
        gf, compat ? "qwen3tts.talker.n_code_groups" : "qwen3-tts.num_code_groups");
    ...
    for (int g = 0; g < cw->num_acoustic_codebooks; g++) {
        char name[160];
        snprintf(name, sizeof(name), compat ? "code_pred.token_embd.%d.weight" : "code_pred.codec_embd.%d.weight", g);
        cw->codec_embedding[(size_t) g] = gf_load_tensor(&wctx, gf, name);
        snprintf(name, sizeof(name), compat ? "code_pred.output.%d.weight" : "code_pred.lm_head.%d.weight", g);
