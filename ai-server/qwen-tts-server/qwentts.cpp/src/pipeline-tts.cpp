...
static bool is_qwen3tts_compat(const GGUFModel & gf) {
    const char * arch = gf_get_str(gf, "general.architecture");
    return arch && std::strcmp(arch, "qwen3tts") == 0;
}

static void parse_codec_specials(const GGUFModel & gf, CodecSpecials & cs) {
    if (is_qwen3tts_compat(gf)) {
        cs.pad_id       = (int) gf_get_u32(gf, "qwen3tts.talker.codec_pad_id");
        cs.bos_id       = (int) gf_get_u32(gf, "qwen3tts.talker.codec_bos_id");
        cs.eos_id       = (int) gf_get_u32(gf, "qwen3tts.talker.codec_eos_token_id");
        cs.think_id     = (int) gf_get_u32(gf, "qwen3tts.talker.codec_think_id");
        cs.nothink_id   = (int) gf_get_u32(gf, "qwen3tts.talker.codec_nothink_id");
        cs.think_bos_id = (int) gf_get_u32(gf, "qwen3tts.talker.codec_think_bos_id");
        cs.think_eos_id = (int) gf_get_u32(gf, "qwen3tts.talker.codec_think_eos_id");
        return;
    }
...
static void parse_text_specials(const GGUFModel & gf, TextSpecials & ts) {
    if (is_qwen3tts_compat(gf)) {
        ts.im_start_id = (int) gf_get_u32(gf, "qwen3tts.im_start_token_id");
        ts.im_end_id   = (int) gf_get_u32(gf, "qwen3tts.im_end_token_id");
        ts.tts_pad_id  = (int) gf_get_u32(gf, "qwen3tts.tts_pad_token_id");
        ts.tts_bos_id  = (int) gf_get_u32(gf, "qwen3tts.tts_bos_token_id");
        ts.tts_eos_id  = (int) gf_get_u32(gf, "qwen3tts.tts_eos_token_id");
        return;
    }
...
static void parse_languages(const GGUFModel & gf, std::vector<LanguageEntry> & out) {
    const bool compat = is_qwen3tts_compat(gf);
    int64_t name_idx = gguf_find_key(gf.gguf, compat ? "qwen3tts.codec_language_names"
                                                     : "qwen3-tts.codec.language_names");
    int64_t id_idx   = gguf_find_key(gf.gguf, compat ? "qwen3tts.codec_language_ids"
                                                     : "qwen3-tts.codec.language_ids");
...
static void parse_speakers(const GGUFModel & gf, std::vector<SpeakerEntry> & out) {
    const bool compat = is_qwen3tts_compat(gf);
    int64_t name_idx = gguf_find_key(gf.gguf, compat ? "qwen3tts.spk_names"
                                                     : "qwen3-tts.codec.speaker_names");
    int64_t id_idx   = gguf_find_key(gf.gguf, compat ? "qwen3tts.spk_token_ids"
                                                     : "qwen3-tts.codec.speaker_ids");
    int64_t dialect_idx =
        compat ? -1 : gguf_find_key(gf.gguf, "qwen3-tts.codec.speaker_dialects");
    if (name_idx < 0 || id_idx < 0 || (!compat && dialect_idx < 0)) {
        return;
    }
    size_t n_names    = gguf_get_arr_n(gf.gguf, name_idx);
    size_t n_ids      = gguf_get_arr_n(gf.gguf, id_idx);
    size_t n_dialects = compat ? n_names : gguf_get_arr_n(gf.gguf, dialect_idx);
    if (n_names != n_ids || (!compat && n_names != n_dialects)) {
    ...
    for (size_t i = 0; i < n_names; i++) {
        ...
        e.name    = gguf_get_arr_str(gf.gguf, name_idx, i);
        e.id      = (int) ids[i];
        e.dialect = compat ? "" : gguf_get_arr_str(gf.gguf, dialect_idx, i);
        out.push_back(e);
    }
}
...
static void parse_generation_defaults(const GGUFModel & gf, GenerationDefaults & g) {
    if (is_qwen3tts_compat(gf)) {
        ...
        g.top_p                 = 1.0f;
        g.temperature           = 0.9f;
        g.repetition_penalty    = 1.05f;
        g.subtalker_do_sample   = true;
        g.subtalker_top_k       = 50;
        g.subtalker_top_p       = 1.0f;
        g.subtalker_temperature = 0.9f;
        g.max_new_tokens        = 2048;
        return;
    }
...
bool pipeline_tts_load(PipelineTTS * pt,
    ...
    const bool compat = arch && std::strcmp(arch, "qwen3tts") == 0;
    if (!arch || (std::strcmp(arch, "qwen3-tts") != 0 && !compat)) {
        qt_log(QT_LOG_ERROR, "[Pipeline] talker GGUF has wrong architecture '%s', expected 'qwen3-tts' or 'qwen3tts'",
               arch ? arch : "");
    ...
    if (compat) {
        pt->tokenizer_type  = "qwen3_tts_tokenizer_12hz";
        pt->model_size      = "0b6";
        pt->model_type      = gf_get_str(pt->gguf_talker, "qwen3tts.tts_model_type");
        pt->num_code_groups = (int) gf_get_u32(pt->gguf_talker, "qwen3tts.talker.n_code_groups");
    } else {
    ...
    // KV caches, one set per slot: kiosk utterances use a compact 1024
    // position talker context. Predictor holds one frame of 16 sub-steps
    // per slot.
    if (!kv_cache_init(&pt->talker_kv, pt->talker.num_hidden_layers, pt->talker.num_key_value_heads,
                       pt->talker.head_dim, 1024, pt->max_batch, pt->backend)) {
        ggml_backend_sched_free(pt->sched);
        pt->sched = NULL;
        pipeline_codec_free(&pt->codec);
        code_predictor_weights_free(&pt->code_predictor);
...
struct TtsSlot {
    ...
    int                  max_new_tokens;  // request limit clamped to remaining KV capacity
...
bool tts_engine_admit(TtsEngine * e, TtsJob * job) {
    ...
    // Prefill occupies T_ctx rows. Frame zero reuses the prefill logits,
    // while every later frame appends one KV row, so at most
    // max_seq_len - T_ctx + 1 frames can be emitted without overflow.
    const int kv_frame_capacity = pt->talker_kv.max_seq_len - s.prompt.T_ctx + 1;
    if (kv_frame_capacity <= 0) {
        qt_set_error("pipeline_tts_synthesize: prompt context %d leaves no generation room in %d-token KV cache",
                     s.prompt.T_ctx, pt->talker_kv.max_seq_len);
        qt_log(QT_LOG_ERROR, "[Pipeline] Prompt context %d exceeds usable %d-token KV cache", s.prompt.T_ctx,
               pt->talker_kv.max_seq_len);
        e->slots.pop_back();
        return tts_admit_fail(job, QT_STATUS_INVALID_PARAMS);
    }
    s.max_new_tokens = params->max_new_tokens < kv_frame_capacity ? params->max_new_tokens : kv_frame_capacity;
    if (s.max_new_tokens < params->max_new_tokens) {
        qt_log(QT_LOG_INFO, "[Pipeline] max_new_tokens clamped from %d to %d for %d-token KV cache",
               params->max_new_tokens, s.max_new_tokens, pt->talker_kv.max_seq_len);
    }
    ...
    s.all_codes.reserve((size_t) s.max_new_tokens);
    s.talker_history.reserve((size_t) s.max_new_tokens);
            ...
            if (!code_predictor_step(&pt->code_predictor, pt->backend, &gs.prefill, gs.steps.data(), c0s.data(), N,
                ...
                for (int i = 0; i < N; i++) {
                    ...
                    if (p->dump_dir && s.step == 0) {
                        ...
                        embed_row_from_gguf(pt->gguf_talker,
                                            is_qwen3tts_compat(pt->gguf_talker) ? "talker.token_embd.weight"
                        ...
                        for (int g = 0; g < num_codebooks - 1; g++) {
                            ...
                            snprintf(name, sizeof(name),
                                     is_qwen3tts_compat(pt->gguf_talker) ? "code_pred.token_embd.%d.weight"
                                                                        : "code_pred.codec_embd.%d.weight",
                                     g);
