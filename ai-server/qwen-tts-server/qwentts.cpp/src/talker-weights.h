...
static bool talker_weights_load(TalkerWeights * tw, const GGUFModel & gf, ggml_backend_t backend) {
    ...
    tw->hidden_size = (int) gf_get_u32(gf, compat ? "qwen3tts.talker.d_model"
                                                  : "qwen3-tts.talker.embedding_length");
    tw->intermediate_size = (int) gf_get_u32(gf, compat ? "qwen3tts.talker.ff_dim"
                                                        : "qwen3-tts.talker.feed_forward_length");
    tw->num_hidden_layers = (int) gf_get_u32(gf, compat ? "qwen3tts.talker.n_layers"
                                                       : "qwen3-tts.talker.block_count");
    tw->num_attention_heads = (int) gf_get_u32(gf, compat ? "qwen3tts.talker.n_heads"
                                                          : "qwen3-tts.talker.attention.head_count");
    tw->num_key_value_heads = (int) gf_get_u32(gf, compat ? "qwen3tts.talker.n_kv_heads"
                                                          : "qwen3-tts.talker.attention.head_count_kv");
    tw->head_dim = (int) gf_get_u32(gf, compat ? "qwen3tts.talker.head_dim"
                                              : "qwen3-tts.talker.attention.key_length");
    tw->vocab_size = (int) gf_get_u32(gf, compat ? "qwen3tts.talker.vocab_size"
                                                : "qwen3-tts.talker.vocab_size");
    tw->rope_theta = gf_get_f32(gf, compat ? "qwen3tts.talker.rope_theta"
                                          : "qwen3-tts.talker.rope.freq_base");
    tw->rms_norm_eps = gf_get_f32(gf, compat ? "qwen3tts.talker.rms_norm_eps"
                                            : "qwen3-tts.talker.attention.layer_norm_rms_epsilon");
    tw->mrope_interleaved = gf_get_bool(gf, compat ? "qwen3tts.talker.rope_interleaved"
                                                  : "qwen3-tts.talker.rope.mrope_interleaved");

    std::vector<uint32_t> mrope =
        gf_get_array_u32(gf, compat ? "qwen3tts.talker.mrope_section" : "qwen3-tts.talker.rope.mrope_section");
    if (mrope.size() == 3) {
    ...
    tw->codec_embedding = gf_load_tensor(&wctx, gf, compat ? "talker.token_embd.weight" : "talker.codec_embd.weight");
    tw->text_embedding =
        gf_load_tensor(&wctx, gf, compat ? "talker.token_embd_text.weight" : "talker.text_embd.weight");
    ...
    tw->codec_head_w    = gf_load_tensor(&wctx, gf, compat ? "talker.output.weight" : "talker.codec_head.weight");
